...
# ============================================
DIVISION = "Cadets"  # Options: "Cadets", "Juniors", "World Ranking", "Olympic"
TOLERANCE = 0.02  # Points difference tolerance

FILE1 = r"Ranking Validation/Feb Ranking All.xlsx"
FILE2 = r"Ranking Validation/Kyorugi_Ranking_February_2026.xlsx"
...
def get_sheet_mapping_file1(division):
    """Map division name to File 1 sheet name"""
    division_map = {
        "Cadets": "Cadets",
        "Juniors": "Juniors",
...
def get_sheet_keyword_file2(division):
    """Get keyword to search for division sheets in File 2"""
    # File 2 typically has sheets like:
    # - "M-58 kg | Cadet Division"
    # - "M-58 kg | Junior Division"
    # - "M-58 kg | World Ranking Senior Division"
    # - "M-58 kg | Olympic Senior Divisi" (truncated)
    
    keyword_map = {
        "Cadets": "Cadet",
        "Juniors": "Junior",
        "World Ranking": "World Senior Division",
        "Olympic": "Olympic"
    }
    return keyword_map.get(division, division)
...
file1_sheet = get_sheet_mapping_file1(DIVISION)
print(f"Reading '{file1_sheet}' sheet from File 1...")

try:
    df1 = pd.read_excel(FILE1, sheet_name=file1_sheet)
except Exception as e:
    print(f"ERROR: Could not read sheet '{file1_sheet}' from File 1")
    print(f"Error: {e}")
    xls1 = pd.ExcelFile(FILE1)
    print(f"Available sheets in File 1: {xls1.sheet_names}")
    exit(1)
...
division_keyword = get_sheet_keyword_file2(DIVISION)
relevant_sheets = [sheet for sheet in xls2.sheet_names if division_keyword.lower() in sheet.lower()]

print(f"Searching for sheets containing: '{division_keyword}'")
print(f"Found {len(relevant_sheets)} relevant sheets in File 2")

if len(relevant_sheets) == 0:
    print(f"ERROR: No sheets found matching keyword '{division_keyword}'")
    print(f"Available sheets in File 2: {xls2.sheet_names}")
    exit(1)
...
for sheet_name in relevant_sheets:
    ...
    # Extract weight category from sheet name (format: "M-58 kg | Division Name")
